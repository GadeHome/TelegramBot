...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        my_message = await send_text(update, context, "Происходит генерация вашего профиля. Подождите пару секунд...")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        my_message = await send_text(update, context, "Происходит генерация сообщения. Подождите пару секунд...")
        answer = await chatgpt.send_question(prompt, user_info)
        await my_message.edit_text(answer)
...
app = ApplicationBuilder().token('8097212465:AAEwOz4B2QmApkookdNQ9bxTuatdtQNOCe4').build()
